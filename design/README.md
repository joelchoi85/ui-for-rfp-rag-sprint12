...
**브랜드 색은 로고에서 뽑았다.** `public/logo.png` 의 보라 `#6500C3` 가 `--color-violet-600` 이고,
나머지 단계는 그 색상각(271°)으로 만들었다. 라이트는 600 을 그대로 채움색으로 쓰고,
다크는 500 으로 한 단계 올린다 — 600 은 검은 바탕에서 경계가 1.99:1 이라 배경에 녹는다.

**출처 색을 브랜드 색과 분리했다.** 브랜드는 violet, 출처는 sky.
...
```
PRIMITIVE   --color-violet-600     원시값. 거의 안 바꾼다
    ↓
